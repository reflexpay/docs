...
> **Base URL:** `https://<switch-host>/api` (all paths below omit this prefix)
---

## Table of Contents

1. [What Switch Is](#1-what-the-switching-service-is)
...
### Application
The top-level entity in Switch - it represents your service. Each application gets:
...
### Subject
A user identity within your application. A subject maps your internal user ID (`externalId`) to the Switch system. One subject can own many tags.
...
- [ ] **Respond with 2xx** within 10 seconds on webhook delivery; Switch retries 3 times
...
| `transactionWebhookUrl` | Recommended | Your HTTPS endpoint that Switch POSTs `TRANSACTION_INITIATED` events to |
...
> - `webhookSecret` - used to verify `X-Switch-Signature` on every inbound webhook; never shown again
...
All protected endpoints require a Bearer JWT. Tokens expire after 1 hour (configurable via `SWITCH_APP_TOKEN_TTL`).
...
A subject represents one of your users inside Switch. Create a subject for every user who needs a tag.
...
Save the returned `id` (Switch subject UUID) - you need it to create a tag.
...
| `subjectId` | ✅ | Switch subject UUID - must belong to your application |

Switch automatically derives the **qualified address** by appending your application's `appHandle`: `alice` → `alice@walletapp`.
...
| `callbackUrl` | Optional | Your HTTPS URL for the outcome callback - overrides your registered `transactionWebhookUrl` for this specific transaction's sender callback. If omitted, Switch falls back to the sending application's registered `transactionWebhookUrl`. |
...
1. Switch persists the transaction in `AWAITING_ACCEPTANCE` state
2. Switch persists a `TransactionWebhook` record (direction=`RECEIVER`, status=`PENDING`)
3. Switch publishes a `WebhookDispatchEvent` to the `switch.webhooks` Kafka topic
...
Switch POSTs notifications to your `transactionWebhookUrl`. You must:
1. Expose a publicly reachable HTTPS endpoint
2. Verify the `X-Switch-Signature` header (see [§15](#15-webhook-signature-verification))
3. Return `2xx` within **10 seconds**; Switch retries up to **3 times** (after 1 s and 2 s back-off)
...
```http
POST https://shop-app.example.com/switch/webhook
Content-Type: application/json
X-Switch-Signature: v0=3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d4
X-Switch-Timestamp: 1741082580
...
Sent to the sending application's transaction-specific `callbackUrl` when provided; otherwise Switch falls back to the sending application's registered `transactionWebhookUrl` after the transaction outcome is determined.

```http
POST https://wallet-app.example.com/switch/callback
Content-Type: application/json
X-Switch-Signature: v0=abc123...
X-Switch-Timestamp: 1741082700
...
✅ Verify X-Switch-Signature before processing (see §15)
✅ Reject if |now - X-Switch-Timestamp| > 5 minutes (replay protection)
...
Only the **receiving application** can accept or reject. Switch enforces this - calling accept/reject with the wrong token returns `403`.
  ...
  "data": [
    {
      ...
      "targetUrl": "https://shop-app.example.com/switch/webhook",
      "status": "DELIVERED",
      "attempts": 1,
    ...
    },
    {
      ...
      "targetUrl": "https://wallet-app.example.com/switch/callback",
      "status": "DELIVERED",
      "attempts": 1,
...
| `FAILED` | All 3 attempts exhausted; event dead-lettered to `switch.webhooks.dlt` |
...
Switch automatically runs an end-of-day settlement job at **23:59 UTC** every day. It aggregates all completed transactions for the day and produces per-application net positions.
...
### 14.1 Encrypting Requests Sent TO Switch

```
1. Fetch the Switch public key (cache it)
2. Serialize your JSON request body to a string
3. Encrypt using JWE (alg=ECDH-ES+A256KW, enc=A256GCM) with the Switch public key
4. Set headers:
...
**Fetch the Switch public key (no auth required):**
...
```json
{
  "data": {
    ...
    "usage": "Encrypt request payloads sent TO Switch. Set 'Content-Encryption: JWE' header."
  }
}
```
...
```java
// 1. Load Switch public key from GET /v1/keys/switch
ECKey switchPublicKey = ECKey.parse(jwkJson);
...
jwe.encrypt(new ECDHEncrypter(switchPublicKey));
...
// 1. Load Switch public key
const publicKey = await importSPKI(switchPemKey, 'ECDH-ES+A256KW');
...
# 1. Load Switch public key
switch_key = ECKey.import_key(jwk_dict)

# 2. Encrypt
protected = {"alg": "ECDH-ES+A256KW", "enc": "A256GCM", "cty": "application/json"}
token = jwe.encrypt_compact(protected, json.dumps(body).encode(), switch_key)
...
### 14.2 Decrypting Webhooks Received FROM Switch
...
After rotation:
- Switch encrypts all subsequent webhooks with the **new** public key
...
After rotation:
- Switch signs all subsequent webhooks with the **new** secret
...
Every outbound webhook POST from Switch includes two headers:

```
X-Switch-Signature: v0=3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8...
X-Switch-Timestamp: 1741082580
```
...
function verifyWebhook(request, webhookSecret):
    timestamp = request.header("X-Switch-Timestamp")
    signature = request.header("X-Switch-Signature")
...
When the limit is exceeded, Switch returns:
...
Switch sends a time-limited consent notification to the tag owner. The consent token URL is:
...
App A requests claim on tag "bob"
         │
         ▼
Switch generates consentToken
Switch notifies "bob" (via email if configured)
         │
         ▼
...
On approval, Switch returns an attestation JWT:
...
Store this token - it proves your application has an approved link to that tag. You can verify it with the Switch public key.
...
Many applications that integrate Switch already run their own tag / username system. When they onboard their users onto Switch, tag collisions are inevitable: *WalletApp* may already have a user `alice`, and so does *ShopApp* - completely different people.

A single global `UNIQUE(tag_string)` constraint would block one application from onboarding. Switch solves this with **qualified addressing**.
...
These are **different identities** sharing a local name - Switch tracks them separately with no constraint conflict.

### What stays the same for the host app

Your application continues to use its own tag system internally. Switch only requires that you **onboard the tags you want to be discoverable cross-platform**:

1. Register on Switch with your `appHandle`
...
4. Switch records `alice@walletapp` as the globally unique qualified address
...
Tags that are **not onboarded** are simply not discoverable via Switch. This is intentional - you control which users participate in the cross-platform network.
...
The second form works only when `alice` is unique on the platform and `bob` is unique on the platform. If either is ambiguous, Switch returns **409** and lists the qualified addresses to use.
...
Switch issues a challenge; your application must verify ownership on the external provider, then call:
...
All Switch responses use a consistent `ApiResponse<T>` envelope:

```json
{
  "success": false,
  "message": "Validation failed",
  "errorCode": "SWITCH-VALID-2001",
  "errors": ["amount must be greater than 0", "currency is required"],
  "timestamp": "2026-03-27T11:45:12.772Z"
}
```

`errorCode` is the stable machine-readable identifier (for example `SWITCH-AUTH-1002`).
Client applications should branch handling logic by `errorCode`, not by free-text `message`.

### Complete error code catalog

All Switch errors are returned with a stable error code in the `errorCode` field. Client applications should parse and branch on this code rather than free-text messages, enabling reliable structured error handling.

#### Authentication & Authorization (1000–1999)

| Code | HTTP | Message | Cause | Resolution |
|------|------|---------|-------|------------|
| `SWITCH-AUTH-1001` | 401 | Authentication required | Missing/invalid `Authorization` header | Provide Bearer token: `Authorization: Bearer <token>` |
| `SWITCH-AUTH-1002` | 401 | Invalid JWT | Token is malformed, expired, or has invalid signature | Re-authenticate via `POST /v1/auth/token` |
| `SWITCH-AUTH-1003` | 401 | Token has been revoked | Application was deactivated after token issuance | Re-authenticate with active application credentials |
| `SWITCH-AUTH-1004` | 401 | Authentication failed | Client ID/secret mismatch, malformed Basic auth | Verify credentials and application is active |
| `SWITCH-AUTH-1005` | 401 | Application not found | Application in JWT subject does not exist | Verify application ID and registration status |
| `SWITCH-AUTH-1006` | 403 | Access denied | Authenticated but unauthorized for this operation | Use correct role/credentials for operation |
| `SWITCH-AUTH-1007` | 403 | Transaction is not addressed to the calling application | Receiver attempting to act on sender's transaction | Use credentials of the receiving application |
| `SWITCH-AUTH-1008` | 403 | Identity link is not associated with the calling application | Application does not own this resource | Use credentials of the owning application |

#### Validation Errors (2000–2999)

| Code | HTTP | Message | Cause | Resolution |
|------|------|---------|-------|------------|
| `SWITCH-VALID-2001` | 400 | Validation failed | Field-level validation errors (required fields, format, length, etc.) | Fix request payload per `errors` list and retry |
| `SWITCH-VALID-2002` | 400 | keyword is required | Search endpoint called without `keyword` query param | Provide non-empty `keyword` parameter |
| `SWITCH-VALID-2003` | 400 | keyword must be at least 2 characters | Search keyword is too short | Use `keyword` with ≥ 2 characters |
| `SWITCH-VALID-2004` | 415 | Unsupported media type | `Content-Type` not `application/json`, or JWE payload misformatted | Send JSON with `Content-Type: application/json`; for JWE set `Content-Encryption: JWE` |
| `SWITCH-VALID-2005` | 400 | Decryption failed | JWE payload could not be decrypted or is malformed | Verify encryption key and payload format |

#### Resource Not Found (3000–3999)

| Code | HTTP | Message | Cause | Resolution |
|------|------|---------|-------|------------|
| `SWITCH-RES-3001` | 404 | Application not found | Application UUID does not exist | Verify application UUID; register if needed |
| `SWITCH-RES-3002` | 404 | Subject not found | Subject UUID does not exist in calling application | Verify subject ID; create via `POST /v1/subjects` if needed |
| `SWITCH-RES-3003` | 404 | Subject not found with external ID | Subject with given external ID does not exist | Verify external ID; create subject if needed |
| `SWITCH-RES-3004` | 404 | Tag not found | Tag (qualified address or bare name) cannot be resolved | Verify tag exists; use qualified address `tag@appHandle` if bare name is ambiguous |
| `SWITCH-RES-3005` | 404 | Claim not found | Claim UUID does not exist | Verify claim ID |
| `SWITCH-RES-3006` | 404 | Claim not found with consent token | Consent token is invalid or expired | Request a new claim to get fresh consent token |
| `SWITCH-RES-3007` | 404 | IdentityLink not found | Identity link UUID does not exist or was revoked | Verify identity link ID |
| `SWITCH-RES-3008` | 404 | IdentityLink not found with challenge token | Challenge token is invalid, expired, or already used | Request new federated identity challenge |
| `SWITCH-RES-3009` | 404 | FederatedIdentity not found | No federated identity exists for this tag | Create federated identity via `POST /v1/identities/federate` |
| `SWITCH-RES-3010` | 404 | Transaction not found | Transaction reference does not exist | Verify transaction reference |
| `SWITCH-RES-3011` | 404 | ApplicationWallet not found | Application wallet not auto-provisioned (rare) | Contact support; wallet should auto-create on first transaction |
| `SWITCH-RES-3012` | 404 | Resource not found | Invalid endpoint path | Verify endpoint path matches API documentation |

#### Resource Conflicts (4000–4999)

| Code | HTTP | Message | Cause | Resolution |
|------|------|---------|-------|------------|
| `SWITCH-CONF-4001` | 409 | Tag already exists | Tag with this qualified address already exists globally | Use different tag name in your application |
| `SWITCH-CONF-4002` | 409 | Your application already has a tag with this name | Tag string already exists in your application | Use different tag name or reuse existing |
| `SWITCH-CONF-4003` | 409 | Subject already exists with this external ID | Subject with this external ID already exists in app | Use unique external ID or fetch existing subject |
| `SWITCH-CONF-4004` | 409 | Subject already has an active claim on this tag | Subject is already linked to tag via active claim | Revoke existing claim if you need to re-link |
| `SWITCH-CONF-4005` | 409 | A pending claim for this subject/tag already exists | Pending consent request for this subject/tag in progress | Wait for pending claim to be approved/denied or revoke it |

#### Invalid State / Business Logic (5000–5999)

| Code | HTTP | Message | Cause | Resolution |
|------|------|---------|-------|------------|
| `SWITCH-STATE-5001` | 422 | Cannot claim a disabled tag | Attempting to claim a tag with `status = DISABLED` | Enable tag via `PATCH /v1/tags/{tag}/enable` or use active tag |
| `SWITCH-STATE-5002` | 422 | Subject does not belong to the calling application | Subject is owned by a different application | Use a subject owned by your application |
| `SWITCH-STATE-5003` | 422 | Consent token has expired | Consent token is past expiration (default: 48 hours) | Request new claim for fresh consent token |
| `SWITCH-STATE-5004` | 422 | Challenge is no longer valid | Identity link challenge in terminal state (USED, DENIED, EXPIRED) | Request new federated identity challenge |
| `SWITCH-STATE-5005` | 422 | Only ACTIVE or PENDING claims can be revoked | Attempting to revoke claim in REVOKED or DENIED state | Only active/pending claims can be revoked |
| `SWITCH-STATE-5006` | 422 | New owner must have an active claim on this tag before transfer | Tag transfer target does not have active claim | Ensure new owner has approved claim before transferring |
| `SWITCH-STATE-5007` | 409 | Transaction is not in AWAITING_ACCEPTANCE state | Attempting to accept/reject transaction not pending | Check transaction status; only AWAITING_ACCEPTANCE can be acted on |
| `SWITCH-STATE-5008` | 422 | Transaction has expired | Attempting to accept transaction past expiration time | Transaction cannot be acted on; check with receiver if needed |

#### Rate Limiting (6000–6999)

| Code | HTTP | Message | Cause | Resolution |
|------|------|---------|-------|------------|
| `SWITCH-RATE-6001` | 429 | Rate limit exceeded. Retry after [N] seconds. | Too many requests for this endpoint/IP/app | Use `Retry-After` header for exponential backoff; default limits: 10 req/min on `/v1/auth/token`, 60 req/min on `/v1/transactions`, 300 req/min on others |

#### System / Internal Errors (9000–9999)

| Code | HTTP | Message | Cause | Resolution |
|------|------|---------|-------|------------|
| `SWITCH-SYS-9001` | 500 | An unexpected error occurred | Unhandled exception in application logic | Retry with exponential backoff; contact support if persistent |
| `SWITCH-SYS-9002` | 400 | JWE decryption error | JWE payload is malformed or uses wrong encryption parameters | Verify encryption key and payload format |
...
| `500` | Server error | Retry with back-off; contact Switch support if persistent |
...
| Method | Path | Auth | Description |
|---|---|---|---|
| `GET` | `/v1/keys/switch` | Public | Switch public key (JWK + Base64 DER) |
...
App A sends TXN-zzz
         ...
         App B does not respond within 24 hours
                   │
                   ▼ Switch scheduled job (every 5 min)
...
```
App A (encryptionEnabled=true)         Switch
──────────────────────────────         ─────

GET /v1/keys/switch ────────────────►
                   ◄────────────────  Switch public key JWK

Serialize request body to JSON
Encrypt with Switch public key (JWE)
POST /v1/transactions ─────────────►
  Headers:
    Content-Encryption: JWE
    Authorization: Bearer <token>
  Body: eyJ...JWE compact string...
                                      JweRequestDecryptionFilter:
                                        ...
                                        decrypts with Switch private key
