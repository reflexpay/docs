...
<CardGroup cols={2}>
  ...
  <Card title="Debit Wallet" icon="arrow-down-wide-short">
    `POST /sso/transaction/debit` - Move funds from customer wallet to partner wallet
  </Card>
  <Card title="Credit Wallet" icon="arrow-up-wide-short">
    `POST /sso/transaction/credit` - Move funds from partner wallet to customer wallet
  </Card>
</CardGroup>
...
Debit a signed-in customer wallet and transfer the value into your partner wallet.

**Endpoint**: `POST /sso/transaction/debit`
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| amount | number | Yes | Amount to debit (minimum: 1) |
| comment | string | No | Transfer narration (max 250 chars) |
| reference | string | No | Your custom reference (max 100 chars) |
...
curl --request POST \
  ...
  --data '{
    "amount": 5000,
    "comment": "Wallet top-up via agent",
    "reference": "DEBIT-9001",
    "metadata": {
      "source": "agent_portal"
    }
  }'
...
Returns a transaction detail object including `Reference`, `Status`, `Sender`, `Recipient`, and settlement values like `Amount`, `Fee`, and `Net`.

---

## Credit Wallet

Credit a signed-in customer wallet from your partner wallet balance.

**Endpoint**: `POST /sso/transaction/credit`
...
Uses the same payload as wallet debit (`amount`, optional `comment`, optional `reference`, optional `currency`, optional `metadata`).

### Request Example

```bash cURL
curl --request POST \
  --url https://staging-api.yourflexpay.com/v2/sso/transaction/credit \
  --header 'Content-Type: application/json' \
  --header 'x-client-id: your_partner_id' \
  --header 'x-api-key: your_api_key' \
  --header 'Authorization: Bearer customer_access_token' \
  --data '{
    "amount": 2500,
    "comment": "Promotional bonus",
    "reference": "CREDIT-1042",
    "metadata": {
      "campaign": "Q3-reward"
    }
  }'
...
Returns a transaction detail object with the same shape as the debit response.
